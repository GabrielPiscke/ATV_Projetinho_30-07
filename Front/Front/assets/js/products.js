document.addEventListener("DOMContentLoaded", async function () {
  ...
  async function renderProducts(products) {
    ...
    for (const product of products) {
      ...
      try {
        ...
        console.warn(
          `Erro ao buscar fotos do produto ${product.id}:`,
          fotoError
        );
      }
      ...
      img.height = 80;
      img.onerror = () =>
        (img.src = "https://via.placeholder.com/100x100?text=Erro");
      ...
      descCell.textContent =
        product.textoDescritivo || product.descricao || "Sem descrição";
      ...
      estoqueCell.textContent =
        product.quantidade != null ? product.quantidade : "";
  ...
  function filterProducts(searchTerm) {
    ...
    return allProducts.filter(
      (product) =>
        ...
        (product.fabricante &&
          product.fabricante.toLowerCase().includes(term)) ||
  ...
  tableBody.addEventListener("click", async function (e) {
    ...
    if (e.target.classList.contains("delete-btn")) {
      ...
      window.location.href = `/Front/pages/edit-product.html?id=${id}`;
    }
  });
